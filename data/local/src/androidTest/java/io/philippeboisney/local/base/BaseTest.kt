...
abstract class BaseTest: KoinTest {
    ...
    protected val database: MercaiAppDatabase by inject()
    ...
    private fun configureLocalModuleTest(context: Context) = module {
        single(DATABASE) {
            Room.inMemoryDatabaseBuilder(context, MercaiAppDatabase::class.java)
                .allowMainThreadQueries()
                .build()
        }
        factory { (get(DATABASE) as MercaiAppDatabase).userDao() }
    }
}